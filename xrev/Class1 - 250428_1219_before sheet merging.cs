            ...
            using (Transaction tx = new Transaction(doc, "Create Black Override Filters"))
            {
                ...
                foreach (ViewSheet sheet in sheetsCollector)
                {
                    ...
                    BuiltInCategory[] mustInclude = new[]
                    {
                        ...
                        BuiltInCategory.OST_FilledRegion, //***** Wall Hach Pattern Category
                    ...
                    //******
                    ParameterFilterElement existingSheetFilter = new FilteredElementCollector(doc)
                        .OfClass(typeof(ParameterFilterElement))
                        .Cast<ParameterFilterElement>()
                        .FirstOrDefault(f => f.Name.Equals(filterName, StringComparison.OrdinalIgnoreCase));

                    while (existingSheetFilter != null)
                    {
                        filterName = $"{baseName}_{counter}";
                        existingSheetFilter = new FilteredElementCollector(doc)
                            .OfClass(typeof(ParameterFilterElement))
                            .Cast<ParameterFilterElement>()
                            .FirstOrDefault(f => f.Name.Equals(filterName, StringComparison.OrdinalIgnoreCase));
                        counter++;
                    ...
                    sheetOGS.SetCutForegroundPatternColor(new Color(0, 0, 0));
                    ...
                    foreach (ElementId viewId in placedViewIds)
                    {
                        ...
                        string viewFilterName = viewBaseName;
                        int viewCounter = 1;

                        // Check for existing
                        ParameterFilterElement existingViewFilter = new FilteredElementCollector(doc)
                            .OfClass(typeof(ParameterFilterElement))
                            .Cast<ParameterFilterElement>()
                            .FirstOrDefault(f => f.Name.Equals(viewFilterName, StringComparison.OrdinalIgnoreCase));

                        while (existingViewFilter != null)
                        {
                            viewFilterName = $"{viewBaseName}_{viewCounter}";
                            existingViewFilter = new FilteredElementCollector(doc)
                                .OfClass(typeof(ParameterFilterElement))
                                .Cast<ParameterFilterElement>()
                                .FirstOrDefault(f => f.Name.Equals(viewFilterName, StringComparison.OrdinalIgnoreCase));
                            viewCounter++;
                        ...
                        viewOGS.SetCutBackgroundPatternColor(new Color(0, 0, 0));
                        viewOGS.SetCutForegroundPatternColor(new Color(0, 0, 0));
